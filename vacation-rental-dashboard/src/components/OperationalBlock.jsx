...
/**
 * Bloque C: Operativa del Día + Calendario 7 días
 */
      ...
      <div className="card">
        ...
        <div className="space-y-3">
          {todayOperations.length === 0 ? (
            <p className="text-gray-500 text-sm">Sin operaciones programadas para hoy.</p>
          ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-bold uppercase ${isCheckIn ? 'text-green-600' : 'text-orange-600'}`}>
                  ...
                  <div className={`flex items-center gap-1 text-xs font-medium ${op.cleaningStatus === 'completed' ? 'text-green-600' : 'text-amber-600'}`}>
                    {op.cleaningStatus === 'completed' ? (
                      <><CheckCircle2 className="w-4 h-4" /> Limpio</>
                    ) : (
          ...
          <table className="w-full text-xs">
            <thead>
              <tr>
                ...
                {next7Days.map((_, i) => (
                  <th key={i} className="pb-3 text-center font-semibold text-gray-500 min-w-[40px]">
                    {getDayLabel(i)}
                  </th>
                ))}
              </tr>
