...
/**
 * Bloque D: Calidad y Reseñas
 * Puntuaciones por plataforma con desglose
 */
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-4 text-sm">
          <div className="flex items-center gap-1">
            ...
            <span className="font-bold text-gray-900">{avgAirbnb}</span>
            <span className="text-gray-500">Airbnb</span>
          </div>
          <div className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-blue-700" />
            <span className="font-bold text-gray-900">{avgBooking}</span>
          ...
          return (
            <div key={q.propertyId} className="border border-gray-100 rounded-lg p-4 hover:shadow-sm transition-shadow">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`badge text-xs ${prop.channel === 'Airbnb' ? 'bg-rose-50 text-rose-700' : 'bg-blue-50 text-blue-700'}`}>
                  {prop.channel}
                </span>
              </div>
              ...
              <div className="flex items-baseline gap-2 mb-3">
                <span className="text-2xl font-bold text-gray-900">
                  {prop.channel === 'Airbnb' ? q.airbnb.score : q.booking.score}
                </span>
                <span className="text-sm text-gray-400">
                  /{prop.channel === 'Airbnb' ? '5' : '10'}
                </span>
                <span className="text-xs text-gray-400 ml-auto">
                  {prop.channel === 'Airbnb' ? q.airbnb.totalReviews : q.booking.totalReviews} reseñas
                </span>
              </div>
